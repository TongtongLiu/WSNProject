...
def analysis(filename):
    ...
    for packet in packets:
        ...
        if packet[3] > max_buffer_usage:
            max_buffer_usage = packet[3]
    valid_packets = 0
    for packet in packets:
        if packet != [0, 0, 0, 0]:
            valid_packets += 1
    ...
    loss_rate = float(sum_resend1to2 + sum_resend2to3) / float(sum_resend1to2 + sum_resend2to3 + valid_packets * 2)
    ...
    print "max_buffer_usage: %f" % max_buffer_usage 
    print "loss_rate1to2: %f" % loss_rate1to2
    print "loss_rate2to3: %f" % loss_rate2to3
    print "loss_rate: %f" % loss_rate
    return filename, data_rate, buffer_usage, max_buffer_usage, loss_rate1to2, loss_rate2to3, loss_rate
